...
func (c *cConfig) LoginConfig(ctx context.Context, _ *api.LoginConfigReq) (res *api.LoginConfigRes, err error) {
	...
	login, err := service.SSysConfig.GetLogin(ctx)
	if err != nil {
		return
	}
...
// Config 获取配置
func (c *cConfig) Config(ctx context.Context, _ *api.ConfigReq) (res *api.ConfigRes, err error) {
...
func (c *cConfig) getWsAddr(ctx context.Context, request *ghttp.Request) string {
	...
	basic, err := service.SSysConfig.GetBasic(ctx)
