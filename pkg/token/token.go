...
// Login 登录
func Login(ctx context.Context, user *contexts.Identity) (string, int64, error) {
	claims := Claims{
		user,
		jwt.RegisteredClaims{},
	}
...
func ParseLoginUser(r *ghttp.Request) (user *contexts.Identity, err error) {
	var (
		ctx    = r.Context()
		header = GetAuthorization(r)
	)
...
func GetTokenKey(appName, authKey string) string {
	return fmt.Sprintf("%v:%v:%v", CacheToken, appName, authKey)
}

// GetBindKey 令牌身份绑定key
func GetBindKey(appName string, userId int64) string {
	return fmt.Sprintf("%v:%v:%v", CacheTokenBind, appName, userId)
}
